...
const OwnerDashboard = ({ onLogout, currentOutlet, products, transactions, userOutlets, onSwitchOutlet }) => {
    ...
    try {
      if (editingEmployee) {
        ...
        setModal({
          ...
          type: 'success',
          title: 'Berhasil!',
          message: 'Karyawan berhasil diperbarui di database!',
          actions: [{ label: 'OK', type: 'primary' }]
        });
        ...
        setModal({
          ...
          type: 'success',
          title: 'Berhasil!',
          message: 'Karyawan berhasil ditambahkan ke database!',
          actions: [{ label: 'OK', type: 'primary' }]
        });
      }
    } catch (error) {
      ...
      setModal({
        ...
        type: 'error',
        title: 'Error',
        message: error?.message || 'Gagal menyimpan karyawan. Pastikan backend sudah running.',
        actions: [{ label: 'OK', type: 'primary' }]
      });
      return;
    }
          ...
          onClick: async () => {
            try {
              ...
              setModal({
                ...
                type: 'success',
                title: 'Berhasil!',
                message: 'Karyawan berhasil dihapus dari database!',
                actions: [{ label: 'OK', type: 'primary' }]
              });
            } catch (error) {
              ...
              setModal({
                ...
                type: 'error',
                title: 'Error',
                message: error?.message || 'Gagal menghapus karyawan dari database',
                actions: [{ label: 'OK', type: 'primary' }]
              });
            }
          }
  ...
  const handleAddOutlet = async () => {
    if (!newOutlet.name || !newOutlet.address || !newOutlet.phone || !newOutlet.email || !newOutlet.city || !newOutlet.province) {
      setModal({
        isOpen: true,
        type: 'error',
        title: 'Validasi Gagal',
        message: 'Nama, alamat, telepon, email, kota, dan provinsi harus diisi!',
        actions: [{ label: 'OK', type: 'primary' }]
      });
      return;
    }
    ...
    if (editingOutlet) {
      // Update outlet - call API
      try {
        const response = await outletAPI.update(editingOutlet.id, newOutlet);
        if (response.success && response.data) {
          ...
          setModal({
            ...
            type: 'error',
            title: 'Gagal!',
            message: response.error || 'Gagal mengupdate outlet',
            actions: [{ label: 'OK', type: 'primary' }]
          });
        }
      } catch (error) {
        ...
        setModal({
          ...
          type: 'error',
          title: 'Error!',
          message: 'Gagal menghubungi server untuk mengupdate outlet',
          actions: [{ label: 'OK', type: 'primary' }]
        });
      }
    } else {
      // Add new outlet - call API
      try {
        const response = await outletAPI.create(newOutlet);
        if (response.success && response.data) {
          // Use data from API response which includes ID from backend
          ...
          setModal({
            ...
            type: 'error',
            title: 'Gagal!',
            message: response.error || 'Gagal menambahkan outlet',
            actions: [{ label: 'OK', type: 'primary' }]
          });
        }
      } catch (error) {
        ...
        setModal({
          ...
          type: 'error',
          title: 'Error!',
          message: 'Gagal menghubungi server untuk menambahkan outlet',
          actions: [{ label: 'OK', type: 'primary' }]
        });
      }
    }

    setNewOutlet({ name: '', address: '', phone: '', email: '', city: '', province: '', postal_code: '' });
  ...
  const handleDeleteOutlet = (id) => {
    setModal({
      ...
      type: 'warning',
      title: 'Konfirmasi Hapus',
      message: 'Yakin ingin menghapus outlet ini? Semua produk di outlet ini akan dihapus!',
      actions: [
        { label: 'Batal', type: 'secondary' },
        {
          label: 'Hapus',
          ...
          onClick: async () => {
            try {
              ...
              setModal({
                ...
                type: 'success',
                title: 'Berhasil!',
                message: 'Outlet berhasil dihapus dari database!',
                actions: [{ label: 'OK', type: 'primary' }]
              });
            } catch (error) {
              console.error('Error deleting outlet:', error);
              const errorMessage = error instanceof Error ? error.message : 'Gagal menghapus outlet dari database';
              setModal({
                isOpen: true,
                type: 'error',
                title: 'Error',
                      ...
                      <div className="add-outlet-form">
                        ...
                        <div className="form-modal">
                          ...
                          <div className="form-group">
                            ...
                            <input
                              type="email"
                              placeholder="Contoh: outlet@madura.com"
                          ...
                          <div className="form-group">
                            ...
                            <input
                              ...
                              value={newOutlet.province}
                              onChange={(e) => setNewOutlet({ ...newOutlet, province: e.target.value })}
                            />
                          </div>
